function Plot_Color_Vessels(M)
  ...
  nColors = 255;
  cMap = M.colorMap;
  plotImage = normalize(adapthisteq(M.xy,'ClipLimit',0.02));
  ...
  if plotLines
    ...
    [vesDiameters] = plot_vessel_diameters_lines(M.VesselData.vessel_list,vesselColormap,maxLineWidth);
  else
    % plot vessels as dots width area and color depending on vessel diameter
    % much faster and kinda looks cooler I think...
    areaScaling = 0.1;
    [vesDiameters] = plot_vessel_diameters(M.VesselData.vessel_list,vesselColormap,areaScaling);
  end
  vesDiameters = vesDiameters*M.dR*1e3;
